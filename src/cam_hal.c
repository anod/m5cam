...
// Debug output that works on USB-CDC (routes through Arduino's log_printf wrapper)
extern int log_printf(const char *format, ...);
#define CAM_DBG(fmt, ...) log_printf("[cam_hal] " fmt "\n", ##__VA_ARGS__)
...
// [M5CamS3-5MP FIX] Disable PSRAM DMA mode — the pre-compiled ll_cam.c was
// compiled without CONFIG_CAMERA_PSRAM_DMA, so its GDMA setup doesn't support
// direct PSRAM writes. Instead we use non-PSRAM mode with an enlarged event
// queue to prevent overflow.
static volatile bool g_psram_dma_mode = false;
...
// Diagnostic counters exposed to main.cpp
static volatile uint32_t g_event_total = 0;
static volatile uint32_t g_vsync_count = 0;
static volatile uint32_t g_eof_count = 0;
static volatile uint32_t g_ovf_count = 0;
static volatile uint32_t g_soi_fail_count = 0;
static volatile uint32_t g_frame_start_count = 0;
static volatile uint8_t g_first_bytes[8] = {0};

uint32_t cam_hal_get_event_total(void) { return g_event_total; }
uint32_t cam_hal_get_vsync_count(void) { return g_vsync_count; }
uint32_t cam_hal_get_eof_count(void) { return g_eof_count; }
uint32_t cam_hal_get_ovf_count(void) { return g_ovf_count; }
uint32_t cam_hal_get_soi_fail(void) { return g_soi_fail_count; }
uint32_t cam_hal_get_frame_start(void) { return g_frame_start_count; }
const volatile uint8_t* cam_hal_get_first_bytes(void) { return g_first_bytes; }

void IRAM_ATTR ll_cam_send_event(cam_obj_t *cam, cam_event_t cam_event, BaseType_t * HPTaskAwoken)
{
    g_event_total++;
    if (cam_event == CAM_VSYNC_EVENT) g_vsync_count++;
    else if (cam_event == CAM_IN_SUC_EOF_EVENT) g_eof_count++;
                ...
                if (cam_event == CAM_IN_SUC_EOF_EVENT) {
                    ...
                    if (cam_obj->jpeg_mode && cnt == 0) {
                        if (cam_obj->psram_mode) {
                            ...
                            // Capture first bytes for diagnostics before SOI check
                            if (frame_buffer_event->len >= 8) {
                                for (int b = 0; b < 8; b++) g_first_bytes[b] = frame_buffer_event->buf[b];
                            }
...
static esp_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    CAM_DBG(" ll_cam_dma_sizes ret=%d psram_mode=%d jpeg_mode=%d",
               ret, cam_obj->psram_mode, cam_obj->jpeg_mode);
    CAM_DBG(" dma_buffer_size=%d half_buffer_size=%d node_buffer_size=%d half_buffer_cnt=%d",
               (int)cam_obj->dma_buffer_size, (int)cam_obj->dma_half_buffer_size,
               (int)cam_obj->dma_node_buffer_size, (int)cam_obj->dma_half_buffer_cnt);
    CAM_DBG(" recv_size=%d fb_size=%d", (int)cam_obj->recv_size, (int)cam_obj->fb_size);
    if (0 == ret) {
        return ESP_FAIL;
    }
    ...
    CAM_DBG(" dma_node_cnt=%d frame_copy_cnt=%d",
    ...
    CAM_DBG(" fb alloc: size=%d dma_align=%d caps=0x%x psram_free=%d",
               (int)alloc_size, (int)dma_align, (unsigned)_caps,
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
...
#endif
        CAM_DBG(" frame[%d] buf=%p (alloc %d bytes)", x,
                   cam_obj->frames[x].fb.buf, (int)alloc_size);
        ...
        if (cam_obj->psram_mode) {
            ...
            CAM_DBG(" frame[%d] dma descriptors=%p (cnt=%d)", x,
                       cam_obj->frames[x].dma, (int)cam_obj->dma_node_cnt);
    ...
    if (!cam_obj->psram_mode) {
        CAM_DBG(" non-psram: allocating DMA buffer %d bytes (largest DMA block=%d)",
                   (int)cam_obj->dma_buffer_size,
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size, uint16_t sensor_pid)
{
    CAM_CHECK(NULL != config, "config pointer is invalid", ESP_ERR_INVALID_ARG);
    esp_err_t ret = ESP_OK;
    CAM_DBG("*** PATCHED cam_config() entered ***");
...
#endif
    CAM_DBG(" psram_mode=%d jpeg_mode=%d", cam_obj->psram_mode, cam_obj->jpeg_mode);
    cam_obj->frame_cnt = config->fb_count;
    cam_obj->width = resolution[frame_size].width;
    cam_obj->height = resolution[frame_size].height;

    if(cam_obj->jpeg_mode){
        // [M5CamS3-5MP FIX] Use generous JPEG receive buffer. The default AUTO
        // formula (w*h/5) is far too small for PY260 JPEG output at high
        // resolutions. 512KB handles even FHD JPEG comfortably while allowing
        // double-buffering within the 8MB PSRAM budget.
        cam_obj->recv_size = 512 * 1024;
        ESP_LOGI(TAG, "*** PATCHED recv_size=%d ***", (int)cam_obj->recv_size);
    ...
    // [M5CamS3-5MP FIX] The default queue_size (dma_half_buffer_cnt - 1) is only
    // 1 in non-PSRAM JPEG mode, causing immediate DMA overflow. Override with a
    // value based on recv_size to give cam_task enough headroom.
    size_t queue_size = cam_obj->dma_half_buffer_cnt - 1;
    if (queue_size == 0) {
        queue_size = 1;
    }
    if (cam_obj->jpeg_mode && !cam_obj->psram_mode && cam_obj->dma_half_buffer_size > 0) {
        size_t needed = cam_obj->recv_size / cam_obj->dma_half_buffer_size;
        if (needed > queue_size) {
            queue_size = needed;
        }
    }
    ...
    CAM_DBG(" ll_cam_init_isr ret=0x%x", ret);
